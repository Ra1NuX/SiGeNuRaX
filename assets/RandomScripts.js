TotalNum = 25;
RNG1 = Math.floor((Math.random() * 25) + 1);
RNG2 = Math.floor((Math.random() * 25) + 1);
while (RNG1 == RNG2) {
	RNG2 = Math.floor((Math.random() * 25) + 1);
}
Operador = 2;
...
function GetE(e) {
	...
	if (cont < 2) {
		...
		} else if (document.getElementById(Clicked).style.background != color_correct) {
			document.getElementById(Clicked).style.color = "white";
			document.getElementById(Clicked).style.background = color_correct; TotalNum -= 1;
			document.getElementById("pointsNum").innerHTML = TotalNum; Operador -= 1; cont += 1
		}
	}
	if (cont == 2) {
		setTimeout(() => {alert("SE ACABO!")}, 500);
	}
}
